...
const MainChart = () => {
  ...
  useEffect(() => {
    const fetchPaidLeave = async () => {
      try {
        const response = await axios.get(`${Config.apiUrl}/paidLeaves?empid=${user?.empid}`);
        setPaidLeaves(response.data.data[0].PaidLeave);
      } catch {
        console.error("error fetching paid leaves.");
      }
    };
    fetchPaidLeave();
  }, [user?.empid]);
          ...
          <CCard className="mb-4">
            ...
            <CCardFooter>
              <div style={{ fontSize: '13px' }}>
                ...
                {leavesUsed > paidLeaves && (
                <span style={{ marginLeft: '10px' }}>LWP: {leavesUsed - paidLeaves}</span>                  
                )}
              </div>
            </CCardFooter>
          </CCard>
